...
def itemlist_signature(item_list: dict) -> dict:
    ...
    for element in elements:
        ...
        match = re.search(r'([0-9]+(?:[.,][0-9]+)?)/100', haystack)
...
for lang_key in ("en", "cn"):
    for listing_name in ("ratings.html", "index.html"):
        listing_path = localized_file(listing_name, lang_key)
        ...
        if listing_ids != catalog_ids:
            errors.append(
                f"{listing_path.relative_to(ROOT).as_posix()}: data-research-id order/count "
                f"must match RU canonical corpus ({len(catalog_ids)}); found {len(listing_ids)}."
            )
